...
QString MainWindow::viewRefrigerantManagement(int since)
{
    ...
    QString order_by = m_settings.orderByForView(LinkParser::RefrigerantManagement);
    if (order_by.isEmpty())
        order_by = "date";
...
HTMLTable * MainWindow::writeCustomersTable(const QString & customer_id, HTMLTable * table)
{
    ...
    QString order_by;
    if (!customer_id.isEmpty() || m_settings.orderByForView(LinkParser::AllCustomers).isEmpty())
        order_by = "company ASC, id ASC";
    else
        order_by = appendDefaultOrderToColumn(m_settings.orderByForView(LinkParser::AllCustomers));
...
QString MainWindow::viewCustomer(const QString & customer_id)
{
    ...
    if (m_settings.customerDetailsVisible()) {
        main.newLine();
...
QString MainWindow::viewCircuit(const QString & customer_id, const QString & circuit_id, int year)
{
    ...
    if (m_settings.circuitDetailsVisible()) {
    ...
    QString order_by = m_settings.orderByForView((LinkParser::Customer << Link::MaxViewBits) | LinkParser::Circuit);
    if (order_by.isEmpty())
        order_by = "date";
                                  ...
                                  const QString & bg_class, bool compare_nom, double tolerance, bool expand_text)
{
    ...
    } else if (compare_nom && actionCompare_values->isChecked() && !nom_value.isEmpty() && !ins_value.isEmpty()) {
...
QString MainWindow::viewAllAssemblyRecordTypes(const QString & highlighted_id)
{
    ...
    ListOfVariantMaps items = all_items.listAll("*", m_settings.orderByForView(LinkParser::AllAssemblyRecordTypes));
...
QString MainWindow::viewAllAssemblyRecordItemTypes(const QString & highlighted_id)
{
    ...
    ListOfVariantMaps items = all_items.listAll("*", m_settings.orderByForView(LinkParser::AllAssemblyRecordItemTypes));
...
QString MainWindow::viewAllAssemblyRecordItemCategories(const QString & highlighted_id)
{
    ...
    ListOfVariantMaps items = all_items.listAll("*", m_settings.orderByForView(LinkParser::AllAssemblyRecordItemCategories));
...
QString MainWindow::viewAllCircuitUnitTypes(const QString & highlighted_id)
{
    ...
    ListOfVariantMaps items = all_items.listAll("*", m_settings.orderByForView(LinkParser::AllCircuitUnitTypes));
...
QString MainWindow::viewAllAssemblyRecords(const QString & customer_id, const QString & circuit_id, int year)
{
    ...
    *(_tr->addHeaderCell()->link("allassemblyrecords:/order_by:operator")) << tr("Operator");
    ...
    QString order_by = m_settings.orderByForView(LinkParser::AllAssemblyRecords);
    if (order_by.isEmpty())
        order_by = "date";
